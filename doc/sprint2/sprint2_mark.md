...
## Tracking on JIRA  (max 24 marks) 
  ...
  - Do the pull request on GitHub match the tasks listed on JIRA Active Sprint Board? (max 2 marks)
    ...
    Please create pr and do code review before merge.
...
## Sprint Completion (Max 20 marks) 
  ...
  I noticed that some of your user stories are complete (or almost complete) but not marked as done in Jira. Several of them (create/edit contracts) are lack frontend interface (only giving client a textbox editor for md format contract), but is acceptable for this sprint.
  
  So only SRUM-17 is counted as incomplete as I didn't see it in the demo.
